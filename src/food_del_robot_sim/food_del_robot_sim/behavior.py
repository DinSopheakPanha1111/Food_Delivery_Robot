...
from nav2_msgs.action import NavigateToPose, Wait, BackUp, Spin
...
# Wraps one recovery action client (Wait, BackUp, or Spin),
# monitors every goal sent to it, and records duration + outcome.

class RecoveryTracker:
    """
    Passive tracker: watches a Nav2 recovery action server by sending
    zero-effect goals and immediately cancelling — NO, instead we just
    hook into the action feedback topic by registering a callback on the
    action server's status topic (/wait/_action/status, etc.).

    Actually the cleanest ROS2 approach without interfering with Nav2 is
    to subscribe to the action server's GoalStatusArray topic published
    under <action>/_action/status.
    """

    def __init__(self, node: Node, action_name: str, behavior_name: str):
        self.node = node
        self.action_name = action_name
        self.behavior_name = behavior_name
        self.lock = threading.Lock()

        # Records: list of dicts per trigger
        # { "start_time": float, "end_time": float, "duration": float,
        #   "outcome": "SUCCEEDED" | "ABORTED" | "CANCELED" | "UNKNOWN",
        #   "goal_label": str }
        self.records = []

        # Track active goals: goal_id_str → start_time
        self._active = {}
        ...
        # Current goal label (set externally when nav goal changes)
        self.current_goal_label = "—"

    def set_goal_label(self, label: str):
        with self.lock:
            self.current_goal_label = label

    def _status_cb(self, msg):
        now = time.time()
        with self.lock:
            for status in msg.status_list:
                gid = self._goal_id_str(status.goal_info.goal_id.uuid)
                s   = status.status

                # STATUS_EXECUTING (2) or STATUS_ACCEPTED (1) → goal started
                if s in (GoalStatus.STATUS_EXECUTING, GoalStatus.STATUS_ACCEPTED):
                    if gid not in self._active:
                        self._active[gid] = {
                            "start": now,
                            "label": self.current_goal_label,
                        }

                # Terminal states: SUCCEEDED(4), ABORTED(6), CANCELED(5)
                elif s in (GoalStatus.STATUS_SUCCEEDED,
                           GoalStatus.STATUS_ABORTED,
                           GoalStatus.STATUS_CANCELED):
                    if gid in self._active:
                        entry     = self._active.pop(gid)
                        start     = entry["start"]
                        duration  = now - start
                        outcome_map = {
                        ...
                        outcome = outcome_map.get(s, "UNKNOWN")
                        self.records.append({
                            "goal_label": entry["label"],
                        ...
                        self.node.get_logger().info(
                            f"[Recovery] {self.behavior_name} | "
                            f"{entry['label']} | {outcome} | {duration:.2f}s"
                        )

    @staticmethod
    def _goal_id_str(uuid_bytes) -> str:
        return bytes(uuid_bytes).hex()

    def get_records_for_goal(self, label: str):
        with self.lock:
            return [r for r in self.records if r["goal_label"] == label]
...
class NavGoalSender(Node):
    def __init__(self):
        ...
        # ── Recovery trackers ──────────────────────────────────────────────
        self._wait_tracker   = RecoveryTracker(self, "/wait",   "Wait")
        self._backup_tracker = RecoveryTracker(self, "/backup", "BackUp")
        self._spin_tracker   = RecoveryTracker(self, "/spin",   "Spin")
        self._all_trackers   = [
            self._wait_tracker,
            self._backup_tracker,
            self._spin_tracker,
        ]
    ...
    def run(self):
        ...
        while True:
            ...
            # Tell recovery trackers which goal is active
            for t in self._all_trackers:
                t.set_goal_label(point_label)
            ...
            # Collect recovery records for this goal
            recovery_records = []
            for t in self._all_trackers:
                recovery_records.extend(t.get_records_for_goal(point_label))
            recovery_records.sort(key=lambda r: r["start_time"])

            n_recoveries = len(recovery_records)
            print(f"  {status} — Travel: {travel_time:.2f}s | "
                  f"Replans: {n_replans} | Recoveries: {n_recoveries}")
    ...
    def print_report(self):
        ...
        pose_rows = []
        for r in self.results:
            gx = r["goal_x"]; gy = r["goal_y"]; gt = r["goal_theta"]
            label = r["label"]
        ...
        print(tabulate(replan_rows,
            headers=["Goal", "Plan", "Time into Travel (s)", "Poses", "Type"],
            tablefmt="fancy_grid"))

        # ── TABLE 5: Recovery Behavior Summary ────────────────────────────
        print(f"\n{sep}")
        print("  TABLE 5 — RECOVERY BEHAVIOR SUMMARY")
        print(sep)
        print("  Tracks Wait, BackUp, and Spin behaviors triggered by the behavior tree.")
        print("  Time into Travel (s) = when the behavior started relative to goal travel start.")
        print("  Duration (s)         = how long the behavior ran.")
        print("  Outcome              = SUCCEEDED / ABORTED (failed) / CANCELED.")
        print(sep)

        recovery_rows = []
        for r in self.results:
            records = r["recovery_records"]
            label   = r["label"]

            if not records:
                recovery_rows.append([label, "None", "—", "—", "—", "—"])
                recovery_rows.append(["", "", "", "", "", ""])
                continue

            # Count per behavior type
            wait_count   = sum(1 for rec in records if rec["behavior"] == "Wait")
            backup_count = sum(1 for rec in records if rec["behavior"] == "BackUp")
            spin_count   = sum(1 for rec in records if rec["behavior"] == "Spin")
            fail_count   = sum(1 for rec in records if rec["outcome"] != "SUCCEEDED")

            recovery_rows.append([
                label,
                f"{len(records)} total "
                f"(Wait:{wait_count} BackUp:{backup_count} Spin:{spin_count}  "
                f"Failed:{fail_count})",
                "—", "—", "—", "—",
            ])

            travel_start = r["travel_time"]  # use as reference base
            # Compute time_into_travel from stored start_time relative to first plan
            first_plan_time = r["goal_plans"][0]["time_into_travel"] if r["goal_plans"] else 0.0

            for i, rec in enumerate(records):
                # time_into_travel: we stored absolute start_time in tracker
                # but we also stored travel_start_time. Compute offset.
                # Actually we use records sorted by start_time; show index offset
                outcome_symbol = (
                    "✓ SUCCEEDED" if rec["outcome"] == "SUCCEEDED"
                    else "✗ ABORTED"  if rec["outcome"] == "ABORTED"
                    else "— CANCELED"
                )
                recovery_rows.append([
                    "",
        ...
        # If no recovery happened at all
        if all(not r["recovery_records"] for r in self.results):
            print("  No recovery behaviors were triggered during any goal.\n")
        else:
            print(tabulate(recovery_rows,
                headers=["Goal", "Behavior / Summary", "Duration (s)",
                         "—", "—", "Outcome"],
                tablefmt="fancy_grid"))
