...
def error_percent(measured, reference):
    """Percentage error of measured vs reference (Euclidean distance)."""
    if reference == 0.0:
        return 0.0
...
class NavGoalSender(Node):
    def __init__(self):
        ...
        # ── Odometry subscriber — try /odom/wheel first, fall back to /odom ─
        self.current_odom_pose = None
        self._odom_source = None
        self._odom_lock = threading.Lock()

        # Subscribe to both; whichever publishes first wins
        ...
        # ── Result storage (one entry per goal attempt) ────────────────────
        # Each entry is a dict with all captured data
        self.results = []

        self.get_logger().info("NavGoalSender ready.")

    # ── Odom callbacks — first source wins ────────────────────────────────
    ...
    def _snapshot_pose(self, pose):
        """Return a plain dict snapshot of a pose so it is not mutated later."""
        if pose is None:
            return None
        return {
            "x":   pose.position.x,
    ...
    def run(self):
        ...
        # Give odometry a moment to arrive so source is detected
        time.sleep(2.0)
        ...
        while True:
            ...
            # Store everything for the final report
            self.results.append({
                "label":       point_label,
                "goal_x":      x,
                "goal_y":      y,
